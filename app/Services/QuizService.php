...
class QuizService implements QuizServiceInterface
{
    ...
    public function generateQuizFromAI(int $studyCardId, array $options = []): Quiz
    {
        ...
        // 2. Cek apakah sudah ada soal di bank untuk study card ini
        $bankQuestions = $this->getBankQuestions($studyCardId);
        
        // Jika belum ada bank soal untuk study card ini, generate via AI sekali saja
        if (empty($bankQuestions)) {
            Log::info('No bank questions found, generating via AI (one-time)', [
        ...
        // Jika bank soal sudah ada, gunakan dari bank (tidak generate ulang)
        Log::info('Using questions from existing bank', [
            'study_card_id' => $studyCardId,
            'available' => count($bankQuestions),
            'requested' => $numQuestions
        ]);
        
        // Ambil soal sesuai jumlah yang diminta, atau semua jika tidak cukup
        $selectedQuestions = array_slice($bankQuestions, 0, min($numQuestions, count($bankQuestions)));
        
        return $this->createQuizFromBank($studyCard, $selectedQuestions, $options);
    }

    /**
     * Generate soal via AI dan simpan ke bank soal (hanya sekali)
     */
    private function generateAndSaveToBankOnce(StudyCard $studyCard, int $numQuestions, array $options = []): Quiz
    {
        // Extract konten materi (text atau dari PDF)
...
Material:\n\n" . $materialContent;
        ...
        // Pilih AI provider (default: gemini)
        ...
        // Buat quiz master sebagai container bank soal
        $quiz = $this->repository->create([
            'study_card_id'        => $studyCard->id,
            'title'                => $studyCard->title . ' - Bank Soal',
            'description'          => 'Master question bank generated by AI from ' . $studyCard->material_type . ' material',
        ...
        // Simpan SEMUA soal ke bank soal (generate sekali, pakai berkali-kali)
        $questionOrder = 1;
        foreach ($questionsData as $questionData) {
            $question = $quiz->questions()->create([
                ...
                'is_bank_question' => true, // ✅ Ini adalah bank soal master
                'topic'         => $studyCard->title,
                'difficulty_level' => 'medium',
                'usage_count'   => 0, // Belum dipakai
                'last_used_at'  => null,
            ]);
    ...
    /**
     * Buat quiz instance baru dari soal-soal yang ada di bank
     */
    private function createQuizFromBank(StudyCard $studyCard, array $bankQuestions, array $options = []): Quiz
    {
        // Buat quiz instance baru (bukan bank, tapi referensi ke bank)
        $quiz = $this->repository->create([
            'study_card_id'        => $studyCard->id,
        ...
        // Copy soal dari bank ke quiz instance
        $questionOrder = 1;
        foreach ($bankQuestions as $bankQuestion) {
            $question = $quiz->questions()->create([
                ...
                'is_bank_question' => false, // ✅ Ini adalah instance quiz, bukan master bank
                'topic'         => $bankQuestion['topic'],
                'difficulty_level' => $bankQuestion['difficulty_level'],
            ]);

            // Copy answers dan acak
            $answers = $bankQuestion['answers'];
            shuffle($answers); // Acak jawaban agar tidak selalu sama urutannya
            ...
            // Update usage count di bank soal master
            \App\Models\QuizQuestion::where('id', $bankQuestion['id'])
